...
public class NGram {
	...
	public void computeNGramsFromStringArray() {
		for (int i=0; i<wordsarr.length-n+1; i++) {
			String ngram = "";
			for (int j=0; j<n-1; j++) {
				ngram += wordsarr[i+j] + " ";
			}
	...
	private void calculateProbabilities() {
		...
		for (Gram g : table.values()) {
			numberOfNgrams += g.occurrences;
		}
		final int numOfNGrams = numberOfNgrams;
		
		table.forEach((key, value) -> {
			value.probability = (value.occurrences / numOfNGrams) * 100.0;
			list.add(value);
		});
	}
	
	public ArrayList<Gram> getListOfNGrams() {
		return list;
	}
