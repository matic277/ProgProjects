...
public class DataInfo implements Serializable {
  ...
  public String getValueOfField(String fieldName) {
  	  // some data might have white-spaces, due to
  	  // values not taking all the value length available to them
  	  // so, trim
  ...
  public String recordToString() {
	  // trim values due to white-space
	  StringBuilder sb = new StringBuilder();
	  for (int i=0; i<values.length - 1; i++) sb.append(values[i].trim() + "^");
	  sb.append(values[values.length - 1].trim());
	  return sb.toString();
  }
}
